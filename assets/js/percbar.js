...
export class PercentageBar {
  ...
  createPart (serverRegistration) {
    ...
    div.setAttribute('Kaytrack-server-id', serverRegistration.serverId)
  ...
  handleMouseOver = (event) => {
    const serverId = parseInt(event.target.getAttribute('kaytrack-server-id'))
    ...
    this._app.tooltip.set(event.target.offsetLeft, event.target.offsetTop, 10, this._parent.offsetTop + this._parent.offsetHeight + 10,
      ...
      ${formatNumber(serverRegistration.playerCount)} Online<br>
